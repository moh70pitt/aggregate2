...
public class PAWSUMInterfaceV2 implements UMInterface {
  // private static final int LEVELS = 9; // define how many leves will be for
  // each content item. First level is knowledge, then progress, then ..
  private static final int LEVELS = 12; // modified by @Jordan as we added three metrics for measuring performance on
                                        // the last k attempts
  ...
  private HashMap<String, double[]> kcSummary; // knowledge components (concepts) and the level of knowledge of the user
                                               // in each of them
  private HashMap<String, ArrayList<String[]>> kcByContent; // for each content there is an array list of kc (concepts)
                                                            // with id, weight (double) and direction
   ...
   * @ return each double[]: knowledge, progress, attempts/loads, success rate,
   * completion, annotation count, like count, time spent, sub activities
   */
  public HashMap<String, double[]> getContentSummary(String usr, String grp, String sid, String cid, String domain,
    ...
    for (Map.Entry<String, String[]> content : contentList.entrySet()) {
      ...
      if (umSvcURLs.get(urls[0]) == null)
        umSvcURLs.put(urls[0], urls[0]);
      if (activitySvcURLs.get(urls[1]) == null)
        activitySvcURLs.put(urls[1], urls[1]);

      // if(contentLists4UM.get(urls[0]) == null)
      // contentLists4UM.put(urls[0],content_name);
      // else
      // contentLists4UM.put(urls[0],contentLists4UM.get(urls[0])+","+content_name);
      //
      // if(contentLists4ACT.get(urls[1]) == null)
      // contentLists4ACT.put(urls[1],content_name);
      // else
      // contentLists4ACT.put(urls[1],contentLists4ACT.get(urls[1])+","+content_name);

      if (contentByProvider.get(content_provider) == null)
        contentByProvider.put(content_provider, "\"" + content_name + "\"");
      else
        contentByProvider.put(content_provider,
            contentByProvider.get(content_provider) + "," + "\"" + content_name + "\"");
    }
    ...
    for (Map.Entry<String, String> calls : umSvcURLs.entrySet()) { //
      ...
      if (!svcURL.equalsIgnoreCase("none") && !svcURL.equalsIgnoreCase("unknown") && !svcURL.equalsIgnoreCase("NA")) {
        ...
        for (Map.Entry<String, String[]> provider : providers.entrySet()) {
          if (provider.getValue()[0] != null && provider.getValue()[0].equals(svcURL)
              && contentByProvider.get(provider.getKey()) != null) {
            json += "        {\"provider-id\" : \"" + provider.getKey() + "\", \"content-list\" : ["
                + contentByProvider.get(provider.getKey()) + "]},\n";
          }
        }
        json = json.substring(0, json.length() - 2);
        json += "\n    ]\n}";

        // Make a call to svcURL and process data adding content to contentSummary if
        // content does not exist already, or complete the corresponding element
        // @@@
    ...
    for (Map.Entry<String, String> calls : activitySvcURLs.entrySet()) {
      ...
      if (jsonResponse != null) {
        ...
        for (int i = 0; i < jsonContentArray.length(); i++) {
          ...
          double subs = 0;
          try {
            progress = c.getDouble("progress");
          } catch (Exception e) {
          }
          ;
          try {
            att = c.getDouble("attempts");
          } catch (Exception e) {
          }
          ;
          try {
            sr = c.getDouble("success-rate");
          } catch (Exception e) {
          }
          ;
          try {
            anns = c.getDouble("annotation-count");
          } catch (Exception e) {
          }
          ;
          try {
            likes = c.getDouble("like-count");
          } catch (Exception e) {
          }
          ;
          try {
            time = c.getDouble("time-spent");
          } catch (Exception e) {
          }
          ;
          try {
            subs = c.getDouble("sub-activities");
          } catch (Exception e) {
          }
          ;
  ...
  /*
   * New version of the method contains more detailed information of the attempts.
   * Also allows to call the method with no content list (contentList = null)
   */
  public HashMap<String, Activity> getContentSummaryV2(String usr, String grp, String sid, String cid, String domain,
    ...
    if (contentList != null) {
      ...
      for (Map.Entry<String, String[]> provider : providers.entrySet()) {
        ...
        if (activitySvcURLs.get(urls[1]) == null)
          activitySvcURLs.put(urls[1], urls[1]);
        if (contentByProvider.get(provider.getKey()) == null)
          contentByProvider.put(provider.getKey(), "");
      }
    }
    ...
    for (Map.Entry<String, String> calls : activitySvcURLs.entrySet()) {
      ...
      if (jsonResponse != null) {
        ...
        for (int i = 0; i < jsonContentArray.length(); i++) {
          ...
          ;
          try {
            attemptSeq = c.getString("attempts-seq");
          } catch (Exception e) {
            attemptSeq = "";
          }
          ;
          try {
            lastKprogress = c.getDouble("lastk-progress");
          } catch (Exception e) {
            lastKprogress = 0;
          }
          ;
          try {
            lastKatt = c.getDouble("lastk-attempts");
          } catch (Exception e) {
            lastKatt = -1.0;
          }
          ;
          try {
            lastKsr = c.getDouble("lastk-success-rate");
          } catch (Exception e) {
            lastKsr = -1.0;
          }
          ;
  ...
  /*
   * This implementation is temporal. It should use the URL of services defined
   * for each content provider.
   * And these services should extend the protocol to deliver activity record in
   * "expanded" form, given
   * for example, an extra parameter
   * e.g., ArrayList<Attempt> lastAttempts =
   * um_interface.getLastContentActivity(usr, grp, sid, cid, domain, contentList,
   * providers, null, (lastUpdateDate != null ? lastUpdateDate : "2016-01-01"));
   * 
   */
  ...
  public ArrayList<Attempt> getLastContentActivity(
    ...
    String svcURL = server + "/aggregateUMServices/GetLastActivity" + "?usr=" + usr + "&domain=" + domain + "&dateFrom="
        + dateFrom;
  ...
  private static JSONObject callService(String url, String json) {
    ...
    // A JSON object is created to pass the required parameter to the recommendation
    // service implemented by GetRecommendations.java
    try {
  ...
  public static JSONObject readJsonFromStream(InputStream is) throws Exception {
    ...
    try {
      ...
      // e.printStackTrace();
      if (verbose)
        System.out.println("  error parsisng json from this service");
    }
    return json;
  }
