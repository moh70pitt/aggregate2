...
public class Aggregate {
   ...
   * @param updateUM true: build the user model, false: build a null user model if
   *                 the user has no precomputed model stird in DB
   * @param cm       a ConfigManager object contains configuration variables
   */
  ...
  public void computeUserLevels(String usr, String grp, String sid, String cid, String domain) {
    ...
    // fill the hash map with the knowledge(see note below) and progress
    // computations from UM interface contentSummary,
    // each double[]: knowledge(see note below), progress,
    // attempts/loads, success, rate, completion, other 1, other 2
    ...
    // Knowledge of each item is aggregated based on knowledge in related KCs
    // (if userKCLevels is computed)
    if (cm.agg_kcmap) {
  ...
  // student in the list
  // Pass computePeerLevels as true to get all, or false to get only the user
  // model
  public void fillClassLevels(boolean includeNullStudents, boolean removeZeroProgressStudents,
      boolean computePeerLevels) {
    ...
    if (subgroups != null && subgroups.size() > 0) {
      for (String[] subgroup : subgroups) {
        ...
        if (!subgroupName.contains("_performance")) {
          ...
          // cskamil: TODO: The names below (e.g., higher performance/progress was given
          // without thinking
  ...
  public void updatePointsAndBadges(String last_content_id, String last_content_res, String last_content_seq,
    ...
    // check last progress with new progress to prevent give student point for
    // repetitive solving
    if (Double.parseDouble(last_content_res) > Double.parseDouble(last_content_old_progress)) {
      ...
      agg_db.insertRecentPointToHistoryTable(usr, grp, String.valueOf(last_content_point), description,
          String.valueOf(newTotalPoint), last_content_id, last_content_provider);
      ...
      // this function check the existance of previous badges and may have not been
  ...
  public void checkExistanceOfPreviousBadges(List<Badges> allBadges, int TotalRec, int TotalCo, int TotalCh,
      int TotalEx) {
  ...
  public void fillRecommendations(String last_content_id, String last_content_res, int n) { // @@@ get the content
                                                                                            // provider
  ...
  public void sendStudentModelUpdateRequest(String last_content_id, String last_content_res) {
    ...
    if ((cm.agg_proactiverec_enabled && cm.agg_proactiverec_method.equals("bng")) ||
      ...
      if (result >= 0 && result <= 1) {
        ...
        String params = createStudentModelUpdateParamJSON(usr, grp, cid, last_content_id, last_content_res,
            contentsString, event);
  ...
  private String createStudentModelUpdateParamJSON(String usr, String grp, String cid, String lastAct,
  ...
  public void genJSONHeader(FluentMap map) {
    map.put("version", "0.0.3")
        .put("context", map()
            .put("learnerId", usr)
            .put("group", map()
                .put("id", grp)
                .put("name", grp_name)))
        .put("reportLevels", array(
            map().put("id", "p").put("name", "Progress"),
            map().put("id", "k").put("name", "Performance")));
    ...
    for (String[] r : resourceList) {
      ...
      resources.add(map()
          ...
          .put("dim", map()
              .put("w", r[6])
              .put("h", r[7]))
          .put("updateStateOn", map()
              .put("done", (r[4].charAt(0) == '1') ? "true" : "false")
              .put("winClose", (r[4].charAt(1) == '1') ? "true" : "false")
              .put("winCloseIfAct", (r[4].charAt(2) == '1') ? "true" : "false")));
    }
  }

  public void genJSONVisProperties(FluentMap map) {
    Map<String, String> userParameterMap = getUserVisParameterMap();
    userParameterMap.putAll(this.userPreferences);

    map.put("vis", map()
        .put("topicSizeAttr", array("difficulty", "importance"))
        .put("color", map()
            .put("binCount", 7)
            .put("value2color", "function (x) { var y = Math.log(x)*0.25 + 1;  return (y < 0 ? 0 : y); }"))
        .put("userManual", userManualFile)
        .put("ui", map()
            .put("params", map()
                .put("group", groupParameters != null && groupParameters[0] != null ? groupParameters[0] : "")
                .put("user", userParameterMap))));
  }
  ...
  public void genJSONConfigProperties(FluentMap map) {
    map.put("configprops", map()
        .put("agg_proactiverec_enabled", cm.agg_proactiverec_enabled)
        .put("agg_proactiverec_threshold", cm.agg_proactiverec_threshold)
        .put("agg_proactiverec_method", cm.agg_proactiverec_method)
        .put("agg_proactiverec_max", cm.agg_proactiverec_max)
        .put("agg_reactiverec_enabled", cm.agg_reactiverec_enabled)
        .put("agg_reactiverec_threshold", cm.agg_reactiverec_threshold)
        .put("agg_reactiverec_method", cm.agg_reactiverec_method)
        .put("agg_reactiverec_max", cm.agg_reactiverec_max)
        .put("agg_line_rec_enabled", cm.agg_line_rec_enabled)
        .put("agg_kc_student_modeling", cm.agg_kcmap_method));
  }

  public void genJSONTopics(FluentMap map) {
    List<Object> $mTopics = new ArrayList<>();
    map.put("topics", $mTopics);

    for (String[] topic : topicList) {
      FluentMap $mActivites = map();
      if (topicContent.get(topic[0]) == null) {
        for (int i = 0; i < resourceList.size(); i++)
          $mActivites.put(resourceList.get(i)[0], array());
      } else {
        List<String>[] content = topicContent.get(topic[0]);
        for (int i = 0; i < content.length; i++) {
          List<Object> activityContents = new ArrayList<>();
          $mActivites.put(resourceList.get(i)[0], activityContents);

          List<String> contentItems = content[i];
          if (contentItems != null && contentItems.size() > 0) {
            for (String item : contentItems) {
              String[] content_data = this.contentList.get(item);

              FluentMap $mActItem = map()
                  .put("id", item)
                  .put("name", content_data[1])
                  .put("url", content_data[2] + (cm.agg_line_rec_enabled ? "&lineRec=1" : "") + "&svc=masterygrids")
                  .put("kcs", array(content_data[6]));
              activityContents.add($mActItem);

              if (this.statsContent != null) {
                ContentStats stats = statsContent.get(item);
                if (stats != null) {
                  $mActItem.extend(stats.asPartialJSON2());
                }
              }
            }
          }
      ...
      $mTopics.add(map()
          .put("id", topic[0])
          .put("name", topic[1])
          .put("difficulty", df.format(getTopicDifficulty(topic[1])))
          .put("importance", df.format(getTopicImportance(topic[1])))
          .put("order", topic[2])
          .put("concepts", array())
          .put("isVisible", topic[3].equalsIgnoreCase("0") ? "false" : "true")
          .put("timeline", map()
              .put("covered", topic[4].equalsIgnoreCase("covered") ? "true" : "false")
              .put("current", topic[4].equalsIgnoreCase("current") ? "true" : "false")
              .put("activities", $mActivites)));
    }
  }

  public void genJSONKCs(FluentMap map) {
    List<Object> $mKCs = new ArrayList<>();
    map.put("kcs", $mKCs);
    if (singleKCList != null) {
      for (Map.Entry<String, KnowledgeComponent> kcEntry : singleKCList.entrySet()) {
        $mKCs.add(kcEntry.getValue().toFluentMap());
      }
    }

    List<Object> $mKCGroups = new ArrayList<>();
    map.put("kcgroups", $mKCGroups);
    if (groupedKCList != null) {
      for (Map.Entry<String, KnowledgeComponentGroup> kcEntry : groupedKCList.entrySet()) {
        $mKCGroups.add(kcEntry.getValue().toFluentMap());
      }
    }
  }

  public void genJSONLearnerPreferences(FluentMap map, String student) {
    Map<String, String> $mUserPrefs = this.groupPreferences.get(student);
    if ($mUserPrefs != null) {
      map.put("preferences", $mUserPrefs);
    }
  }

  public void genJSONLearnerState(FluentMap map, String student) {
    FluentMap $mState = map();
    ...
    FluentMap $mTopics = map();
    $mState.put("topics", $mTopics);

    FluentMap $mActivities = map();
    $mState.put("activities", $mActivities);
    ...
    for (String[] topic : topicList) {
      ...
      FluentMap $mActTopicName = map();
      $mActivities.put(topic_name, $mActTopicName);
      ...
      if (content != null && content.length > 0) {
        ...
        for (int i = 0; i < content.length; i++) {
          ...
          FluentMap $mRsrcName = map();
          $mActTopicName.put(resourceName, $mRsrcName);

          if (contentItems != null && contentItems.size() > 0) {
            for (String item : contentItems) {
              FluentMap $mItem = map();
              $mRsrcName.put(item, $mItem);

              $mItem.put("sequencing", sequencing ? df.format(getContentSequenceScore(item)) : "");

              FluentMap $mValues = map();
              $mItem.put("values", $mValues);

              levels = student_c_l == null ? null : student_c_l.get(item);
              if (levels == null) {
                $mValues.put("k", 0).put("p", 0);
              } else {
                $mValues.put("k", df.format(levels[0]))
                    .put("p", df.format(levels[1]));

                if (student.equalsIgnoreCase(usr) && levels.length > 8) {
                  $mValues.put("a", df.format(levels[2]))
                      .put("s", df.format(levels[3]))
                      .put("t", df.format(levels[7]));

                  if (userContentSequences != null) {
                    String attSeq = userContentSequences.get(item);
                    if (attSeq != null)
                      $mValues.put("aSeq", attSeq);
                  }
                  if (activityConceptCounts != null && activityConceptCounts.size() > 0) {
                    ...
                    $mValues.put("kcsNotKnown", df.format(counts[0]))
                        .put("kcsLearning", df.format(counts[1]))
                        .put("kcsKnown", df.format(counts[2]))
                        .put("difficulty", df.format(diff));
                  }
                }
              }
            }
      ...
      FluentMap $mTopicTopicName = map();
      $mTopics.put(topic_name, $mTopicTopicName);

      if (sequencing) {
        FluentMap $mSeq = map();
        $mTopicTopicName.put("sequencing", $mSeq);

        for (int i = 0; i < resourceList.size(); i++) {
          resourceName = resourceList.get(i)[0];
          $mSeq.put(resourceName, df.format(getTopicSequenceScore(topic_name, resourceName)));
        }
      }

      FluentMap $mValues = map();
      $mTopicTopicName.put("values", $mValues);
      ...
      if (levels != null) {
        ...
        for (int i = 0; i < resourceList.size(); i++) {
          ...
          $mValues.put(resourceName, map()
              .put("k", df.format(levels[2 * i]))
              .put("p", df.format(levels[2 * i + 1])));
        }

        $mTopicTopicName.put("overall", map()
            .put("k", df.format(overallK))
            .put("p", df.format(overallP)));
      } else {
        for (int i = 0; i < resourceList.size(); i++) {
          resourceName = resourceList.get(i)[0];
          $mValues.put(resourceName, map().put("k", df.format(0)).put("p", df.format(0)));
        }
    ...
    if (student.equalsIgnoreCase(usr) && cm.agg_kcmap) {
      ...
      if (allKCList != null) {
        for (Map.Entry<String, KnowledgeComponent> kcEntry : allKCList.entrySet()) {
          ...
          $mKCs.put(
              kcJSONById ? String.valueOf(kc.getId()) : kc.getIdName(),
              map().put("k", df.format(levels[0]))
                  .put("p", df.format(levels[1]))
                  .put("sr", df.format(levels[3]))
                  .put("lastk-att", df.format(levels[10]))
                  .put("lastk-sr", df.format(levels[11])));
        }
      }
    }
  }
  ...
  //
  public void genJSONGroupState(FluentMap map, Map<String, double[]> aggs_t_l, Map<String, double[]> aggs_c_l,
    ...
    for (String[] topic : topicList) {
      ...
      FluentMap $mValues = map();
      $mTopics.put(topic_name, map().put("values", $mValues));

      // level k,p per topic
      if (levels != null) {
        for (int i = 0; i < resourceList.size(); i++) {
          resourceName = resourceList.get(i)[0];
          $mValues.put(resourceName, map().put("k", df.format(levels[2 * i])).put("p", df.format(levels[2 * i + 1])));
        }
      } else {
        for (int i = 0; i < resourceList.size(); i++) {
          resourceName = resourceList.get(i)[0];
      ...
      if (content != null) {
        for (int i = 0; i < content.length; i++) {
          ...
          if (contentItems != null && contentItems.size() > 0) {
            for (String item : contentItems) {
              levels = aggs_c_l == null ? null : aggs_c_l.get(item);

              if (levels != null) {
                $mRsrcName.put(item, map().put("values", map()
                    .put("k", df.format(levels[0]))
                    .put("p", df.format(levels[1]))
                    .put("a", (levels[2] >= 0 ? df.format(levels[2]) : 0))
                    .put("s", (levels[3] >= 0 ? df.format(levels[3]) : 0))
                    .put("t", (levels[7] >= 0 ? df.format(levels[7]) : 0))
                    .put("n", df.format(levels[9]))));
              } else {
                $mRsrcName.put(item, map().put("values", map().put("k", 0).put("p", 0)));
              }
            }
          }
        }
    ...
    if (cm.agg_kcmap) {
      if (allKCList != null) {
        for (Map.Entry<String, KnowledgeComponent> kcEntry : allKCList.entrySet()) {
          ...
          $mKCs.put(kcJSONById ? String.valueOf(kc.getId()) : kc.getIdName(), map()
              .put("k", df.format(levels[0]))
              .put("p", df.format(levels[1])));
        }
      }
    ...
    $mState.put("learnerIds", learner_ids);
  }

  public void genJSONRecommendation(FluentMap map) {
    List<Object> $mRecs = new ArrayList<>();
    map.put("recommendation", $mRecs);

    if (recommendation_list != null && recommendation_list.size() > 0) {
      for (ArrayList<String> rec : recommendation_list) {
        String stored_value = rec.get(6) != null ? rec.get(6) : "-1";

        $mRecs.add(map()
            .put("recommendationId", rec.get(0))
            .put("topicId", rec.get(1))
            .put("resourceId", rec.get(2))
            .put("activityId", rec.get(3))
            .put("score", rec.get(4))
            .put("feedback", map()
                .put("text", rec.get(5))
                .put("storedValue", stored_value)));
      }
    }

    List<Object> $mRecKC = new ArrayList<>();
    if (this.recommendedActivitiesKCModeler != null && recommendedActivitiesKCModeler.size() > 0) {
      ...
      for (ArrayList<String[]> recs : recommendedActivitiesKCModeler) {
        ...
        $mRecKC.add(map().put("model" + i, model));

        for (String[] rec : recs) {
          model.add(map().put("qz", rec[0]).put("ex", rec[1]));
        }
        i++;
      }
    }
  ...
  public void genJSONFeedback(FluentMap map) {
    FluentMap $mFeedback = map();
    map.put("feedback", $mFeedback);

    if (activity_feedback_form_items != null && activity_feedback_form_items.size() > 0) {
      List<Object> $mItems = new ArrayList<>();
      $mFeedback.put("id", activity_feedback_id);
      $mFeedback.put("items", $mItems);

      for (ArrayList<String> fed : activity_feedback_form_items) {
        List<Object> $mResponse = new ArrayList<>();
        $mItems.add(map()
            .put("id", fed.get(0))
            .put("text", fed.get(1))
            .put("type", fed.get(2))
            .put("required", fed.get(3))
            .put("response", $mResponse));
        ...
        if (_response_items != null && _response_items.length > 0) {
          for (int i = 0; i < _response_items.length; i++) {
            ...
            if (_response != null && _response.length == 2) {
              $mResponse.add(map().put("value", _response[0]).put("label", _response[1]));
            }
          }
        }
      }
  ...
  public String genAllJSON(int n, int top) {
    ...
    genJSONHeader(map);
    genJSONVisProperties(map);
    genJSONConfigProperties(map);
    genJSONTopics(map);
  ...
  public String genUserJSON(boolean computeGroupLevels, String last_content_id, String last_content_res) {
    FluentMap map = map()
        .put("lastActivityId", last_content_id)
        .put("lastActivityRes", last_content_res);

    FluentMap learner = map();
    learner.put("id", usr)
        .put("name", usr_name);
    genJSONLearnerState(learner, usr);
    ...
    genJSONRecommendation(map);
    genJSONFeedback(map);
    genLogJSON(map);
    genBadgeJSON(map, false);
    genTotalRecJSON(map);

    return map.toString();
  }

  private void getLearners(FluentMap map, int n) {
    if (!this.includeOthers)
      n = 0;

    List<Object> $mLearners = new ArrayList<>();
    map.put("learners", $mLearners);
    ...
    for (String[] learner : class_list) {
      ...
      if (c < n - 1 || learner[0].equalsIgnoreCase(usr) || n == -1) {
        FluentMap $mLearner = map()
            .put("id", learner[0].equalsIgnoreCase(usr) ? learner[0] : learner[3])
            .put("name", learner[1])
            .put("isHidden", ishidden);

        $mLearners.add($mLearner);
        genJSONLearnerState($mLearner, learner[0]);
        genJSONLearnerPreferences($mLearner, learner[0]);
      }
      c++;
    }
  }

  private void getAggLevels(FluentMap map) {
    List<Object> $mGroups = new ArrayList<>();
    map.put("groups", $mGroups);

    if (subgroups_names != null) {
      for (int g = 0; g < subgroups_names.size(); g++) {
        FluentMap $mGroup = map().put("name", subgroups_names.get(g));
        $mGroups.add($mGroup);

        genJSONGroupState($mGroup, subgroups_topic_levels.get(g), subgroups_content_levels.get(g),
            subgroups_kc_levels.get(g), subgroups_student_anonym_ids.get(g));
      }
    }
  }

  public void genLogJSON(FluentMap map) {
    openDBConnections();

    List<Object> $mLogs = new ArrayList<>();
    map.put("logs", $mLogs);

    List<Logs> recentPoints = agg_db.getFiveRecentPoints(usr, grp);
    if (recentPoints != null) {
      for (Logs log : recentPoints) {
        if (Integer.parseInt(log.getTotalPoint()) > 0)
          $mLogs.add(map()
              .put("value", log.getValue())
              .put("description", log.getDescription())
              .put("totalPoint", log.getTotalPoint()));
      }
    }
  ...
  public void genBadgeJSON(FluentMap map, boolean allOrUser) { // true for mod=all, false for mod=user
    openDBConnections();

    List<Object> $mOwnBadges = new ArrayList<>();
    map.put("ownBadges", $mOwnBadges);
    ...
    if (allOrUser) {
      ...
      for (Badges bdg : badges) {
        $mOwnBadges.add(map()
            .put("id", bdg.getId())
            .put("value", bdg.getValue())
            .put("name", bdg.getName())
            .put("type", bdg.getType())
            .put("img_URL", bdg.getImgURL())
            .put("congradulationMSG", bdg.getCongradualationMSG()));
      }
  ...
  public void genTotalRecJSON(FluentMap map) {
    openDBConnections();

    String totalSolvedRecommendedActivity = agg_db.getTotalRecForEachStudent(usr, grp);
    if (totalSolvedRecommendedActivity != null)
      map.put("rmcCount", map().put("value", totalSolvedRecommendedActivity));
    else
      map.put("rmcCount", map());
