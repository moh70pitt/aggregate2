...
public class Aggregate {
	...
	public static int nContentLevels; // 9 levels (K,P,A,S,C,AN,L,T,Sub) knowledge,progress,N attempts, success rate, completion, annotations, likes, time, subactivities 
	public static int nKCLevels; // 7 levels (K,P,A,SR,S,C,T ) knowledge,progress, attempts (raw), success rate, problems solved, connections done, time
	...
	public Aggregate(String usr, String grp, String cid, String sid,
		...
		if(cm.agg_kcmap){
			// Get the KC from the database. It includes single and multiple components groups
	...
	public Aggregate(String grp, ConfigManager cm) {
		...
		nTopicLevels = resourceList.size() * 2; // the dimension of the topic levels array. For each resource there is a level of knowledge and a level of progress
		nContentLevels = 9; // the dimension of the content levels array. For each resource there is a level of knowledge and a level of progress
	...
	public void computeUserLevels(String usr, String grp, String sid, String cid, String domain) {
		...
		HashMap<String, Activity> hashMapActivity = um_interface.getContentSummaryV2(usr, grp, sid, cid, domain, contentList, providers, null,null);
...
// TODO: 
		...
		if(cm.agg_kcmap){
			KCModeler kcModeler = new KCModeler(usr, domain, cid, singleKCList, groupedKCList, contentList, cm.agg_kcmap_method, domain, cm.servletSource.getServletContext());
...
//				if(userLastModel != null && userLastModel[2] != null && userLastModel[2].length() > 0){
//					lastUpdateDate = userLastModel[3];
//					lastModel = formatLevels(userLastModel[2], nKCLevels, false);
//				}
//				
//
//				// 2. if the model contains kcs (not empty string), then call getContentSummaryV2 passing date, otherwise, pass null date
//				//HashMap<String, Activity> lastActivity = um_interface.getContentSummaryV2(usr, grp, sid, cid, domain, null, providers, null, lastUpdateDate);
//				// @@@@ HERE WE CAN ADD THE ACTIVITIES IN ORDER
//				ArrayList<Attempt> lastAttempts = um_interface.getLastContentActivity(usr, grp, sid, cid, domain, contentList, providers, null, (lastUpdateDate != null ? lastUpdateDate : "2016-01-01"));
//				//System.out.println("####### ###### "+cm.agg_kcmap_method);
//				//userKCLevels = kcModeler.computeKCModel(lastUpdateDate, lastModel, lastAttempts);
//				userKCLevels = kcModeler.computeKCModel((lastUpdateDate != null ? lastUpdateDate : "2016-01-01"), lastModel, hashMapActivity, lastAttempts);
...
//
//				// 2. 
//				ArrayList<Attempt> lastAttempts = um_interface.getLastContentActivity(usr, grp, sid, cid, domain, contentList, providers, null, lastUpdateDate);
//				
//				userKCLevels = kcModeler.computeKCModelComparison(lastUpdateDate, lastModel, hashMapActivity, lastAttempts);
...
//	public void computeUserLevels2(String usr, String grp, String sid, String cid, String domain) {
//		if (usr == null || domain == null || usr.length() == 0 || domain.length() == 0) return;
//
//		userTopicLevels = new HashMap<String, double[]>();
...
//		// contentSummary, each double[]: knowledge, progress, attempts/loads, success rate, completion, other 1, other 2
//		long time1 = Calendar.getInstance().getTimeInMillis();
//		userContentLevels2 = um_interface.getContentSummaryV2(usr, grp, sid, cid, domain, contentList, providers, null, null);
//
//
//		if(verbose) System.out.println("  Get all form UM   " + (Calendar.getInstance().getTimeInMillis()-time1));
//
...
//		// COMPUTE LEVELS FOR KCs
//		if(cm.agg_kcmap){
//			//HashMap<String, Activity> getContentSummaryV2(String usr, String grp, String sid, String cid, String domain, 
...
//
//			// 2. if the model contains kcs (not empty string), then call getContentSummaryV2 passing date, otherwise, pass null date
//			HashMap<String, Activity> activity = um_interface.getContentSummaryV2(usr, grp, sid, cid, domain, 
//					null, providers, null, lastUpdateDate);
//			KCModeler kcModeler = new KCModeler(usr, domain, cid, singleKCList, groupedKCList, cm.agg_kcmap_method);
//			//System.out.println("####### ###### "+cm.agg_kcmap_method);
...
//	}
			...
			if (models != null) {
				...
				if (model4content != null && model4content.length() > 0) {
					if(learnerid.equalsIgnoreCase(this.usr)) {
						//userContentSequences = new HashMap<String, String>();
						HashMap<String, double[]> learner_content_levels = formatLevels(model4content, nContentLevels, true);
						peers_content_levels.put(learnerid, learner_content_levels);
					}else{
						HashMap<String, double[]> learner_content_levels = formatLevels(model4content, nContentLevels, false);
						peers_content_levels.put(learnerid, learner_content_levels);
					}
	...
	public HashMap<String, double[]> formatLevels(String model, int nlevels, boolean lastElementIsAttSeq) {
		...
		for (int i = 0; i < model_arr.length; i++) {
			...
			if(lastElementIsAttSeq){
				...
				if(userContentSequences != null) userContentSequences.put(name, attSeq);
			}
		}
		return res;
	}
	...
	public HashMap<String, double[]> computeSubGroupContentLevels(ArrayList<String> learners){
		...
		for (Map.Entry<String, String[]> content : contentList.entrySet()) {
			String content_name = content.getKey();
			double[] avglevels = new double[nContentLevels + 1]; // one more level to include how many students have attempted
			int div = 0;
			...
			if(attemptedCount==0) attemptedCount = 1;
		...
		for (Map.Entry<String, KnowledgeComponent> kcEntry : allKCList.entrySet()){
			...
			for (String learner : learners) {
				if (non_students.get(learner) == null && !learner.equals(usr)) {
					...
					if (levels == null || levels.length == 0) {
						for(int j=0;j<nKCLevels;j++) avglevels[j] += 0.0;
					} else {
						for(int j=0;j<nKCLevels;j++) avglevels[j] += levels[j];
					}
				}
				div++;
			}
			if(div==0) div = 1;
			for(int j=0;j<nKCLevels;j++) avglevels[j] = avglevels[j]/div;
	...
	public String precomputedKCModel() {
		...
		for (Map.Entry<String, double[]> component : userKCLevels.entrySet()) {
			...
			if (levels != null && levels.length > 0) {
				...
				for(int j=0;j<nKCLevels;j++) user_levels += df2.format(levels[j]) + ",";
				user_levels = user_levels.substring(0, user_levels.length() - 1);
				user_levels += "|";
			}
		}
	...
	public void processParameters (ArrayList<String[]> parameters){
		...
		for(String[] p : parameters){
			if(p[0].equalsIgnoreCase("user")){
				...
				if(p.length>3 && p[3] != null) manualFile = p[3];
			}
			if(p[0].equalsIgnoreCase("group")){
				...
				if(manualFile.length() == 0 && p.length>3 && p[3] != null) manualFile = p[3];
			}
		}
	...
	public void overwriteServiceConfig(String parameters){
		...
		for(String param : params){
			...
			if(pair[0].equalsIgnoreCase("kcMap")) 
				try {cm.agg_kcmap  = pair[1].equalsIgnoreCase("true");} catch(Exception e) {}
			if(pair[0].equalsIgnoreCase("kcMapMethod")) cm.agg_kcmap_method = pair[1].trim();
		...
		for (Map.Entry<String, KnowledgeComponent> kcEntry : allKCList.entrySet()){
			...
			if(kc instanceof KnowledgeComponentGroup) {
				...
				for(KnowledgeComponent innerKc : innerKcs) {
					if(allKCList.get(innerKc.getIdName()) != null) countExisting++;
				}
				if(countExisting == kcG.cardinality()) groupedKCList.put(kc.getIdName(), (KnowledgeComponentGroup)kc);
			}
			else singleKCList.put(kc.getIdName(), kc);
			...
			for (Map.Entry<String, double[]> cEntry : kc.getContents().entrySet()){
				...
				if(contentAttrs != null){
					if(contentAttrs[6].length() == 0) contentAttrs[6] = kc.getId() + "";
					else contentAttrs[6] += "," + kc.getId();
				}
	...
	public void computeConceptCounts(){
		...
		if(student_kc_l == null) return;
		// go through all single KCs and get the level category. Then for each of its activities, update the counting
		...
		for (Map.Entry<String, KnowledgeComponent> kcEntry : singleKCList.entrySet()){
			...
			if(contents != null && !contents.isEmpty()) {
				for (Map.Entry<String, double[]> content : contents.entrySet()){
					...
					if(counts != null) counts[cat] = counts[cat] + 1; // increment 1 the count of the category of the knowledge of the concept
				}
			}
						...
						for (String item : contentItems) {
							...
							if (levels != null) {
								...
								if(student.equalsIgnoreCase(usr) && levels.length>8){
									content_levels += ",\"a\":" + df.format(levels[2]) + ",\"s\":" + df.format(levels[3]) + ",\"t\":" + df.format(levels[7]);
									if(userContentSequences != null){
										String attSeq = userContentSequences.get(item); 
										if(attSeq != null) content_levels += ",\"aSeq\":\"" + attSeq + "\"";
									}
									if(activityConceptCounts != null && activityConceptCounts.size() > 0){
										int[] counts = activityConceptCounts.get(item);
										if(counts == null) counts = new int[3];
										double diff = 0.5;
										if(counts[0] + counts[1] + counts[2] > 0){
											diff = (0*counts[2] + .5*counts[1] + 1*counts[0]) / (counts[0] + counts[1] + counts[2]);
										}
										
										content_levels += ",\"kcsNotKnown\":" + df.format(counts[0]) + ",\"kcsLearning\":" + df.format(counts[1]) + ",\"kcsKnown\":" + df.format(counts[2])+ ",\"difficulty\":" + df.format(diff);
									}
		...
		if(student.equalsIgnoreCase(usr) && cm.agg_kcmap){
			...
			if(allKCList != null){
				for (Map.Entry<String, KnowledgeComponent> kcEntry : allKCList.entrySet()){
					...
					if(kcJSONById) {
						kc_levels += "       " + kc.getId() + ": {\"k\":" + df.format(levels[0]) + ",\"p\":" + df.format(levels[1]) + "},\n";
					}
					else kc_levels += "       \"" + kc.getIdName() + "\": {\"k\":" + df.format(levels[0]) + ",\"p\":" + df.format(levels[1]) + "},\n";
	...
	// 
	public String genJSONGroupState(Map<String, double[]> aggs_t_l, Map<String, double[]> aggs_c_l, Map<String, double[]> aggs_kc_l, ArrayList<String> learner_ids) {
		String res = "  state:{  \n";
						...
						for (String item : contentItems) {
							...
							if (levels != null) {
								...
								content_levels += "          " 
										...
										+ ",\"a\":" + (levels[2] >= 0 ? df.format(levels[2]) : 0) + ",\"s\":" + (levels[3] >= 0 ? df.format(levels[3]) : 0) + ",\"t\":" + (levels[7] >= 0 ? df.format(levels[7]) : 0)
										+ ",\"n\":" + df.format(levels[9])
										+ "}},\n";
								// content_levels += ",\"a\":" + df.format(levels[2]) + ",\"s\":" + df.format(levels[3]) + ",\"t\":" + df.format(levels[7]);
							} else {
								content_levels += "          "
	...
	public String genAllJSON(int n, int top) {
		...
		for(int g=0;g<subgroups_names.size();g++){
			...
			subgroup += genJSONGroupState(subgroups_topic_levels.get(g), subgroups_content_levels.get(g), subgroups_kc_levels.get(g) ,subgroups_student_anonym_ids.get(g)) + "\n},\n";
