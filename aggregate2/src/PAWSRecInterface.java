...
public class PAWSRecInterface implements RecInterface {
  ...
  // "http://localhost:8080/recommendations/GetRecommendations";
  ...
  public ArrayList<ArrayList<String[]>> getRecommendations(String usr,
    ...
    try {
      ...
      String paramsJson = createParamJSON(usr, grp, sid, cid, domain, lastContentId, lastContentResult,
          lastContentProvider,
      ...
      if (verbose)
        System.out.println("RECOMENDATION CALL:");
      if (verbose)
      ...
      if (statusCode != -1) {
        ...
        if (json != null) {
          if (json.has("error")) {
            ...
            String p_recid = "";
            if (reactive.has("id"))
              r_recid = reactive.getString("id");
            if (proactive.has("id"))
              p_recid = proactive.getString("id");

            JSONArray json_reactive_list = null;
            JSONArray json_proactive_list = null;
            if (reactive.has("contentScores"))
              json_reactive_list = reactive.getJSONArray("contentScores");
            if (proactive.has("contentScores"))
              json_proactive_list = proactive.getJSONArray("contentScores");
  ...
  private String createParamJSON(String usr, String grp, String sid, String cid, String domain, String lastContentId,
    ...
    if (maxReactiveRec > -1)
      json.put("reactive_max", maxReactiveRec + "");
    if (maxProactiveRec > -1)
      json.put("proactive_max", maxProactiveRec + "");
    if (reactiveRecThreshold > -1)
      json.put("reactive_threshold", reactiveRecThreshold + "");
    if (proactiveRecThreshold > -1)
      json.put("proactive_threshold", proactiveRecThreshold + "");
    if (reactiveRecMethod != null && reactiveRecMethod.length() > 0)
      json.put("reactive_method", reactiveRecMethod);
    if (proactiveRecMethod != null && proactiveRecMethod.length() > 0)
      json.put("proactive_method", proactiveRecMethod);
  ...
  // private String getTopicContents(HashMap<String, ArrayList<String>[]>
  // topicContent) {
  // String text = "";
  // ArrayList<String>[] contents;
  // for (String t : topicContent.keySet())
  // {
  // text += t + ","; // the first element in each commar separated list is the
  // topic name; topics are separated by ~
  ...
  private String getUserContentProgressText(HashMap<String, double[]> userContentLevels) {
    ...
    if (userContentLevels != null) {
      ...
      if (contentLvl.length() > 0)
        contentLvl = contentLvl.substring(0, contentLvl.length() - 1); // this is for ignoring the last ;
    }
    return contentLvl;
  }
